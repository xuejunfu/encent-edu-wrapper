...
class wrapper:
    ...
    def __fetch_one_ts(self, filename: str):
        ...
        save_dir = os.path.abspath(self.export_dir)
    ...
    def process_directory(self, directory: str):
        ...
        if os.path.exists(dir_path):
            ...
            for file in files:
                ...
                if ext.lower() == '.sqlite':
                    self.process_db_file(filename)
                elif ext.lower() == '.ts':
                    self.process_transcode_mp4(filename)
                else:
                    print('[-]\tfiletype can\'t opt filename: \"{}\".'.format(filename))
                    pass
    ...
    def process_transcode_mp4(self, filename: str):
        print('[-]\ttranscode mp4 start \"{}\".'.format(filename))
        video_clips = [VideoFileClip(filename)]
        final_clip = concatenate_videoclips(video_clips, method='compose')
        save_dir = os.path.abspath(self.export_dir)
        save_filename = os.path.split(filename)[1].lower().replace('.ts', '.mp4')
        save_path = os.path.join(save_dir, save_filename)  # 视频文件保存的完整路径
        print('[-]\ttranscode mp4 save file path \"{}\".'.format(save_path))
        final_clip.to_videofile(save_path, fps=24)
        print('[-]\ttranscode mp4 save file path final \"{}\".'.format(save_path))
